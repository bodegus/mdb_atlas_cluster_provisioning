...
if [ -z "$MODULE_NUM" ]; then
    ...
    echo "Usage: ./tf_apply.sh [module_number]"
    echo ""
...
echo "Running Terraform apply for module: ${MODULE_DIR}"
...
if [ -n "${MONGODB_ATLAS_PROJECT_ID}" ]; then
    terraform apply -var="project_id=${MONGODB_ATLAS_PROJECT_ID}" -auto-approve
else
    terraform apply -auto-approve
fi

echo ""
echo "Terraform apply complete for module ${MODULE_DIR}"
